...
export default function Reservations () {
    ...
    return (
        <main className="w-[100%] flex flex-col items-center space-y-4">
            ...
            <button className="block rounded-md bg-sky-600 hover:bg-indigo-600 px-3 py-2 shadow-sm text-white" onClick={makeReservation}>Reserve this car</button>
            {
                cartItems.map( (items) => <div>{items.returnDate}</div>)
            }
        </main>
    )
}